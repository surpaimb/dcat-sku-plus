(function () {
    ...
    SKU.prototype.init = function () {
        ...
        if (old_val && old_val != "null") {
            ...
            attr_keys.forEach(function (attr_key, index) {
                ...
                switch (scopeAttrType) {
                    ...
                    case "radio":
                        ...
                        let attributeHtml = _this.getAttributeHtml(
                            scopeAttrType,
                            sku[0]
                        );
                        let html = _this.getHtml(
                            attributeHtml,
                            attr_key,
                            scopeAttrType,
                            isFirst
                    ...
                    default:
                        ...
                        let attr_values = { attr_value: attr_vals };
                        let attributeHtml2 = _this.getAttributeHtml(
                            scopeAttrType,
                            attr_values
                        );
                        let html2 = _this.getHtml(
                            attributeHtml2,
                            attr_key,
                            scopeAttrType,
                            isFirst
            ...
            // 修改的部分开始
            let isFirst = true;
            _this.skuAttributes.forEach(function (attribute) {
                let scopeAttrType = attribute.attr_type;
                let attributeHtml = _this.getAttributeHtml(
                    scopeAttrType,
                    attribute
                );
                _this.currentAttributeValue = attribute.attr_values;
                _this.currentAttributeName = attribute.attr_name;
                _this.currentSkuId = attribute.id;
                _this.attrs[attribute.attr_name] = attribute.attr_values;
                let html = _this.getHtml(
                    attributeHtml,
                    _this.currentAttributeName,
                    scopeAttrType,
                    isFirst
                );
                _this.wrap.find(".sku_attr_key_val tbody").append(html);
        ...
        // 绑定属性值添加事件
        _this.wrap
            .find(".sku_attr_key_val")
            .on("click", ".Js_add_attr_val", function () {
                let html =
        ...
        _this.wrap
            .find(".sku_attr_key_val")
            .on("click", ".Js_remove_attr_val", function () {
        ...
        _this.wrap
            .find(".sku_attr_key_val")
            .on("change", ".attribute_selector", function () {
                if ($(this).val() != "input") {
                    let skuAttributesArray = _this.skuAttributes;
                    _this.attrIndex = $(this)
        ...
        _this.wrap
            .find(".sku_attr_key_val")
            .on("click", ".Js_remove_attr_name", function () {
        ...
        _this.wrap
            .find(".sku_attr_key_val tbody")
            .on("change", "input", _this.getSkuAttr.bind(_this));
        _this.wrap
            .find(".sku_edit_wrap tbody")
            .on("keyup", "input", _this.processSku.bind(_this));

        // SKU图片上传
        _this.wrap
            .find(".sku_edit_wrap tbody")
            .on("click", ".Js_sku_upload", function () {
                _this.upload($(this));
            });
        ...
        _this.wrap
            .find(".sku_edit_wrap tbody")
            .on("click", ".sku_img .icon-x", function () {
                let that = $(this);
        ...
        _this.wrap
            .find(".sku_edit_wrap tbody")
            .on("mouseenter", ".sku_img img", function () {
                $(this).next(".icon-x").show();
            });

        _this.wrap
            .find(".sku_edit_wrap tbody")
            .on("mouseleave", ".sku_img img", function () {
        ...
        trs.each(function () {
            ...
            switch (scopeAttrType) {
                ...
                case "radio":
                    ...
                    tr.find(
                        'td:eq(1) input[type="' + scopeAttrType + '"]:checked'
                    ).each(function (i, v) {
                        attr_val.push($(v).val());
                    });
                    break;
    ...
    SKU.prototype.SKUForm = function (default_sku, params) {
        ...
        if (attr_names.length === 0) {
            ...
            thead_html +=
                '<th data-field="price">价格 <input type="text" class="form-control"></th>';
                        ...
                        if (field == "pic" && item_sku[field].length > 0) {
                            ...
                            item_sku[field].forEach(function (v) {
                                html +=
                                    ...
                                    v.full_url +
                                    '"/><i class="feather icon-x" data-path="' +
                                    v.short_url +
                                    '"></i></div>';
                            });
    ...
    SKU.prototype.processSku = function () {
        let _this = this;
        let sku_json = {};
        sku_json.type = _this.wrap
            .find(".sku_attr_select .btn.btn-success")
            .attr("data-type");
        // 多规格
            ...
            case "checkbox":
                if (attribute.attr_value.length > 0) {
                    ...
                    attribute.attr_value.forEach(function (v) {
                        html +=
                            '<div class="vs-checkbox-con vs-checkbox-primary" style="margin-right: 16px"><input value="' +
                            v +
                            '" class="Dcat_Admin_Widgets_Checkbox" type="checkbox" name="' +
                            attribute.attr_name +
                            '"';
                ...
                if (attribute.attr_value.length > 0) {
                    ...
                    attribute.attr_value.forEach(function (v) {
                        html +=
                            ...
                            '<input value="' +
                            v +
                            '" class="Dcat_Admin_Widgets_Radio" type="radio" name="' +
                            attribute.attr_name +
                            '"';
                        if (_this.currentAttributeValue.indexOf(v) >= 0) {
                            html += ' checked="checked"';
    ...
    SKU.prototype.getHtml = function (
        ...
        html +=
            ...
            (innerHtml.length > 0
                ? innerHtml
                : this.getAttributeHtml("input", this.skuAttr)) +
            "</div></td><td>" +
            (isFirst
